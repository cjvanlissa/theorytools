...
```{r eval = knitr::is_html_output(), echo = FALSE, out.width="30%"}
library(tidySEM)
library(ggplot2)
g <- dagitty('dag {
  X [label="Predictor", pos="0,0"]
...
one could use `sample.int(n = 5, size = n)`.
...
g <- dagitty('dag {
  X [exposure,
     pos="0,0",
     label="Study Hours",
     distribution="sample.int(n = 20, size = n)"]
...
graph_sem(g, text_size = 2) -> p
ggsave("dag_three.png", p, device = "png", width = 5, height = 5)
