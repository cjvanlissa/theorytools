...
```{js, class.source = "jsvis1", echo = FALSE, eval = TRUE}
<script>
...
- [ ] Implement a theory represented as a diagram as a FAIR theory
- [ ] Access that FAIR theory in the data analysis environment (i.e., theory reuse)
...
This figure, loosely based on the visual representation of the theory in the paper by Morris and colleagues,
is relatively formalized compared to the standard for developmental psychological theories.
...
If we want to contribute to further theory development, we might instead prefer to [fork the original GitHub repository](https://github.com/github/docs/blob/main/content/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo.md) to our own GitHub account,
and create a local clone of our newly forked repository.
In R, you can clone a repository like so:
...
```{r eval=knitr::is_html_output(), results='asis', echo=FALSE}
theorytools:::quizz(
  "Which of the following describes the property of X-interoperability of a FAIR theory?" = c("It can be reused for a specific operation.", "It can be reused by any person.", "It can be reused for any operation.", "It is licenced to be reused."),
  "If you are contributing to theory development, which platform provides infrastructure to coordinate collaborate with known others and strangers?" = c("GitHub", "Zenodo", "RStudio"))
```
...
Below is a simplistic code snippet to generate synthetic data using the `dagitty::simulateSEM()` function.
...
### Mapping Measured Variables onto Theoretical Constructs

Here, we are using real data to operationalize the theoretical constructs in our DAG.
...
theorytools:::quizz(
  ...
  "Which of these causal effects can we estimate using these data?" = c("All of these ", "PP -> A", "ER  -> PC", "CC -> A"),
  ...
  "Consider the sample size of `df_real`. Could this be related to your answers in the previous two questions?" = c("Yes", "No")
)
```
